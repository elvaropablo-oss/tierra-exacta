...
document.querySelector('#bed-form')?.addEventListener('submit', (event) => {
  ...
  try {
    ...
    reveal('#bed-result', `<p class="result-label">Tierra necesaria</p><h2>${fmt(result.total, 2)} litros</h2><p class="result-equivalent">Equivale a <strong>${fmt(result.total / 1000, 3)} m³</strong></p><dl><div><dt>Por jardinera</dt><dd>${fmt(result.each, 2)} L</dd></div><div><dt>Altura llena</dt><dd>${fmt(result.usableHeight, 1)} cm</dd></div><div><dt>Deducciones totales</dt><dd>${fmt(result.deductionEach * result.count, 2)} L</dd></div></dl><a class="button button--clay" href="${litreLink(result.total)}">Convertir en sacos</a><p class="fineprint">Usa medidas interiores. Las paredes, patas y depósitos integrados reducen el hueco real.</p>`);
